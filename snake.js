...
function drawApple() {
	...
	canvasContext.fillRect(
		applePosition[0].xLocation,
		applePosition[0].yLocation,
		20,
		20
	);
}
...
function checkWallCollision() {
	if (snakeBody[0].xCoord >= canvas.width - 10 || snakeBody[0].xCoord <= 0) {
		endGame();
	}
	if (snakeBody[0].yCoord >= canvas.height - 10 || snakeBody[0].yCoord <= 0) {
		endGame();
	}
}
...
function checkSnakeCollision() {
	for (i = 1; i < snakeBody.length; i++) {
		if (
			snakeBody[0].xCoord === snakeBody[i].xCoord &&
			snakeBody[0].yCoord === snakeBody[i].yCoord
		) {
			endGame();
		}
	}
...
function checkAppleCollision() {
	if (
		snakeBody[0].xCoord === applePosition[0].xLocation &&
		snakeBody[0].yCoord === applePosition[0].yLocation
	) {
		updateGame();
	}
}
